...
senior_dev_cut_off = datetime.datetime.now() - timedelta(days=3*365)
all_senior_developers = list(db.developer.find())
...
for index, element in df_week.iterrows():
    ...
    db.commits_per_chain_1w.insert_one({'chain': element.name[1], 'date': element.name[0], 'commits': int(element["url_x"])})
    
    
db.commits_per_chain_1m.drop()
...
for index, element in df_month.iterrows():
    ...
    db.commits_per_chain_1m.insert_one({'chain': element.name[1], 'date': element.name[0], 'commits': int(element["url_x"])})
