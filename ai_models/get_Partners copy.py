...
openai.api_key = "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"
...
counter = 0
        ...
        df_industries.to_csv("industries.csv", index=False)
    
    df_industries.loc[:,'status'] = 'open'
    df_industries.to_csv("industries.csv", index=False)
    
    df_chains.loc[index, ['status']] = "completed"
    df_chains.to_csv("chains.csv", index=False)
